...
class AttributionBourseListSerializer(serializers.ModelSerializer):
    ...
    etudiant_nom = serializers.CharField(
        source="demande.etudiant.__str__", read_only=True
    )
    type_bourse_nom = serializers.CharField(
        source="demande.type_bourse.nom", read_only=True
    )
...
class AttributionBourseDetailSerializer(serializers.ModelSerializer):
    ...
    def get_total_verse(self, obj):
        return sum(v.montant for v in obj.versements.filter(statut="effectue"))
...
class DemandeBourseDetailSerializer(serializers.ModelSerializer):
    ...
    def get_etudiant(self, obj):
        return {
            ...
            "formation": (
                str(obj.etudiant.formation) if obj.etudiant.formation else None
            ),
        }
