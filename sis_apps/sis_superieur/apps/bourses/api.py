...
from .serializers import (
    TypeBourseListSerializer, TypeBourseDetailSerializer,
    DemandeBourseListSerializer, DemandeBourseDetailSerializer,
    DemandeBourseCreateSerializer, DemandeBourseUpdateSerializer,
    DecisionBourseSerializer, AttributionBourseListSerializer,
    AttributionBourseDetailSerializer, AttributionBourseCreateSerializer,
    VersementBourseSerializer, VersementBourseCreateSerializer,
    StatsBourseSerializer
)
...
class DemandeBourseViewSet(viewsets.ModelViewSet):
    ...
    def creer_attribution(self, request, pk=None):
        ...
        count = AttributionBourse.objects.filter(
...
class AttributionBourseViewSet(viewsets.ModelViewSet):
    ...
    def reactiver(self, request, pk=None):
        ...
        if attribution.statut != 'suspendue':
            return Response({'error': "Seules les attributions suspendues peuvent être réactivées."}, status=400)
    ...
    def versements(self, request, pk=None):
        ...
        if request.method == 'GET':
            serializer = VersementBourseSerializer(attribution.versements.all(), many=True)
            return Response(serializer.data)
        ...
        return Response(
            VersementBourseSerializer(versement).data,
            status=status.HTTP_201_CREATED
        )
    ...
    def generer_versements(self, request, pk=None):
        ...
        return Response({
            ...
            'versements': VersementBourseSerializer(versements_crees, many=True).data
        })
...
class VersementBourseViewSet(viewsets.ModelViewSet):
    ...
    def effectuer_batch(self, request):
        ...
        versements = VersementBourse.objects.filter(
            id__in=ids, statut='planifie'
        )
