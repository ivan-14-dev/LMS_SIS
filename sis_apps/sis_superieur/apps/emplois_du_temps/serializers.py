...
class SalleListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Salle
        fields = [
            'id', 'code', 'nom', 'batiment_code', 'type', 'type_display',
            'capacite', 'disponible'
        ]
...
class SalleCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Salle
        fields = [
            'batiment', 'nom', 'code', 'type', 'capacite', 'etage',
            'equipements', 'accessibilite_pmr', 'disponible', 'departement_gestionnaire'
        ]
...
class CreneauCoursListSerializer(serializers.ModelSerializer):
    ...
    type_display = serializers.CharField(source='get_type_cours_display', read_only=True)

    class Meta:
        model = CreneauCours
        fields = [
            'id', 'jour', 'jour_display', 'horaire', 'ecue_code', 'ecue_nom',
            'enseignant_nom', 'salle_code', 'type_cours', 'type_display', 'groupe'
        ]
...
class CreneauCoursCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CreneauCours
        fields = [
            'semestre', 'formation', 'ecue', 'enseignant', 'salle',
            'jour', 'creneau_horaire', 'type_cours', 'groupe',
            'semaine_debut', 'semaine_fin', 'frequence', 'semaines_paires', 'commentaire'
        ]
    ...
    def validate(self, attrs):
        ...
        if enseignant and creneau:
            conflits = CreneauCours.objects.filter(
                enseignant=enseignant, jour=jour, creneau_horaire=creneau, semestre=semestre
            )
            if instance:
                conflits = conflits.exclude(pk=instance.pk)
            if conflits.exists():
...
class ReservationListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Reservation
        fields = [
            'id', 'salle_code', 'demandeur_nom', 'date', 'heure_debut', 'heure_fin',
...
class ReservationCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Reservation
        fields = ['salle', 'date', 'heure_debut', 'heure_fin', 'motif', 'description', 'nb_personnes']
...
class ConflitHoraireSerializer(serializers.ModelSerializer):
    """Conflit horaire."""
    type_display = serializers.CharField(source='get_type_conflit_display', read_only=True)
