...
from .serializers import (
    ...
    ConflitHoraireSerializer,
    CreneauCoursCreateUpdateSerializer,
    CreneauCoursDetailSerializer,
    CreneauCoursListSerializer,
    CreneauHoraireSerializer,
    ReservationCreateSerializer,
    ReservationDetailSerializer,
    ReservationListSerializer,
    SalleCreateUpdateSerializer,
    SalleDetailSerializer,
    SalleListSerializer,
...
class SalleViewSet(viewsets.ModelViewSet):
        ...
        return Response(
            {
                ...
                "creneaux_reguliers": CreneauCoursListSerializer(
                    creneaux, many=True
                ).data,
    ...
    def disponibilite(self, request, pk=None):
        ...
        if not all([date, heure_debut, heure_fin]):
            return Response(
                {"error": "date, heure_debut et heure_fin sont requis."}, status=400
            )
        ...
        return Response(
            {
                ...
                "conflits": ReservationListSerializer(
                    conflits_reservations, many=True
                ).data,
            }
        )
...
class CreneauCoursViewSet(viewsets.ModelViewSet):
    ...
    def par_formation(self, request):
        ...
        if not formation_id or not semestre_id:
            return Response(
                {"error": "formation_id et semestre_id sont requis."}, status=400
            )
    ...
    def par_enseignant(self, request):
        ...
        return Response({"enseignant_id": enseignant_id, "emploi_du_temps": emploi})
    ...
    def par_salle(self, request):
        ...
        return Response({"salle_id": salle_id, "emploi_du_temps": emploi})
            ...
            if creneau.enseignant:
                ...
                for c in conflits_ens:
                    conflit, _ = ConflitHoraire.objects.get_or_create(
                        ...
                        creneau_2=c,
                        defaults={
                            "description": f"Enseignant {creneau.enseignant} occupé"
                        },
                    )
                    conflits.append(conflit)
...
class ReservationViewSet(viewsets.ModelViewSet):
    ...
    def mes_reservations(self, request):
        ...
        reservations = self.get_queryset().filter(demandeur=request.user.utilisateur)
