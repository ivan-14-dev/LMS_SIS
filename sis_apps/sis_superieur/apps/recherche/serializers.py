...
class LaboratoireListSerializer(serializers.ModelSerializer):
    ...
    directeur_nom = serializers.CharField(source='directeur.user.get_full_name', read_only=True)
    ...
    class Meta:
        ...
        fields = [
            ...
            'type', 'type_display',
            'faculte', 'faculte_nom',
            'directeur_nom', 'tutelle',
            'nb_projets', 'nb_theses',
        ]
...
class LaboratoireDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'tutelle', 'axes_recherche', 'numero_rnsr', 'created_at',
        ]
...
class ProjetRechercheListSerializer(serializers.ModelSerializer):
    ...
    laboratoire_nom = serializers.CharField(source='laboratoire.acronyme', read_only=True)
    responsable_nom = serializers.CharField(source='responsable.user.get_full_name', read_only=True)
...
class ProjetRechercheDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ProjetRecherche
        fields = [
            'id', 'titre', 'acronyme', 'description',
            'laboratoire', 'laboratoire_nom',
            'responsable', 'responsable_nom',
            'membres', 'membres_list',
            'financeur', 'reference', 'budget',
            'date_debut', 'date_fin',
...
class ProductionScientifiqueSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'titre', 'auteurs', 'auteurs_list',
            'laboratoire', 'laboratoire_nom',
            'projet', 'projet_titre',
            'annee', 'doi', 'url_hal',
            'facteur_impact', 'fichier', 'created_at',
        ]
        read_only_fields = ['id', 'created_at']
...
class TheseListSerializer(serializers.ModelSerializer):
    ...
    laboratoire_nom = serializers.CharField(source='laboratoire.acronyme', read_only=True)
    doctorant_nom = serializers.CharField(source='doctorant.user.get_full_name', read_only=True)
    directeur_nom = serializers.CharField(source='directeur.user.get_full_name', read_only=True)
    statut_display = serializers.CharField(source='get_statut_display', read_only=True)
    ...
    class Meta:
        ...
        fields = [
            'id', 'titre',
            'laboratoire', 'laboratoire_nom',
            'doctorant_nom', 'directeur_nom',
            'date_debut', 'date_soutenance',
            'statut', 'statut_display', 'financement',
        ]
...
class TheseDetailSerializer(serializers.ModelSerializer):
    ...
    doctorant_nom = serializers.CharField(source='doctorant.user.get_full_name', read_only=True)
    directeur_nom = serializers.CharField(source='directeur.user.get_full_name', read_only=True)
    co_directeur_nom = serializers.CharField(source='co_directeur.user.get_full_name', read_only=True)
    statut_display = serializers.CharField(source='get_statut_display', read_only=True)
    
    class Meta:
        model = These
        fields = [
            'id', 'titre', 'resume', 'mots_cles',
            'laboratoire', 'laboratoire_nom',
            'doctorant', 'doctorant_nom',
            'directeur', 'directeur_nom',
            'co_directeur', 'co_directeur_nom',
            'date_debut', 'date_soutenance',
            'statut', 'statut_display', 'financement', 'created_at',
        ]
        read_only_fields = ['id', 'created_at']
