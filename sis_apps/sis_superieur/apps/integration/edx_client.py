...
class EdxClient:
    ...
    def _get_access_token(self) -> str:
        ...
        try:
            r = requests.post(url, data=data, timeout=self.timeout, verify=self.verify_ssl)
            r.raise_for_status()
        except requests.HTTPError as e:
            logger.error(f"Échec d'authentification OAuth: {e}")
            raise EdxAuthenticationError(f"Impossible d'obtenir un token OAuth: {e}")
        except requests.RequestException as e:
            logger.error(f"Erreur réseau lors de l'authentification: {e}")
            raise EdxClientError(f"Erreur de connexion au LMS: {e}")
    ...
    def create_user(self, username, email, full_name, is_staff=False, is_superuser=False, role="student", extra=None) -> Dict:
        url = f"{self.lms_url}/api/user/v1/accounts"
        data = {"username": username, "email": email, "name": full_name,
    ...
    def deactivate_user(self, username) -> bool:
        ...
        return requests.post(url, headers=self._headers(), timeout=self.timeout).status_code == 200
    ...
    def get_course(self, course_key) -> Optional[Dict]:
    ...
    def create_course_block(self, course_key, parent_locator, block_type, display_name, category="vertical", metadata=None) -> Dict:
    ...
    def delete_block(self, block_id) -> bool:
        url = f"{self.cms_url}/api/xblock/v1/xblocks/{block_id}"
        return requests.delete(url, headers=self._headers(), timeout=self.timeout).status_code in (200, 204)
    ...
    def enroll_user(self, course_key, username, mode="audit") -> Dict:
        ...
        data = {"course_id": course_key, "username": username, "mode": mode, "is_active": True}
        r = requests.post(url, json=[data], headers=self._headers(), timeout=self.timeout)
        r.raise_for_status()
    ...
    def unenroll_user(self, course_key, username) -> bool:
        ...
        return requests.delete(url, json=data, headers=self._headers(), timeout=self.timeout).status_code in (200, 204)

    def get_enrollments(self, course_key) -> List[Dict]:
        url = f"{self.lms_url}/api/enrollment/v1/enrollment"
        r = requests.get(url, params={"course_id": course_key}, headers=self._headers(), timeout=self.timeout)
        r.raise_for_status()
        return r.json()

    def bulk_enroll(self, course_key, usernames, mode="audit") -> List[Dict]:
        url = f"{self.lms_url}/api/enrollment/v1/enrollment"
        data = [{"course_id": course_key, "username": u, "mode": mode, "is_active": True} for u in usernames]
        r = requests.post(url, json=data, headers=self._headers(), timeout=self.timeout)
        r.raise_for_status()
        return r.json()
    ...
    def post_grade(self, course_key, username, subsection_id, score, max_score=100.0) -> Dict:
    ...
    def add_to_cohort(self, course_key, username, cohort_name) -> Dict:
        ...
        r = requests.post(url, json={"users": [username]}, headers=self._headers(), timeout=self.timeout)
        r.raise_for_status()
        return r.json()
    ...
    def issue_certificate(self, course_key, username, certificate_type="honor") -> Dict:
        ...
        data = {"course_id": course_key, "username": username, "certificate_type": certificate_type}
        r = requests.post(url, json=data, headers=self._headers(), timeout=self.timeout)
        r.raise_for_status()
        return r.json()
    ...
    def register_webhook(self, target_url, event_type, secret) -> Dict:
        ...
        data = {"url": target_url, "event_type": event_type, "secret": secret, "active": True}
        r = requests.post(url, json=data, headers=self._headers(), timeout=self.timeout)
        r.raise_for_status()
        return r.json()
