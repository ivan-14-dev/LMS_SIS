...
class EdxUserMappingTestCase(TestCase):
    ...
    def test_create_mapping(self):
        ...
        mapping = EdxUserMapping.objects.create(
            user_sis=self.user, username_edx="edx-test-user-001"
        )
        assert mapping.username_edx == "edx-test-user-001"
        assert mapping.actif
        assert mapping.user_id_edx is None
        assert mapping.date_sync is None

    def test_mapping_str_representation(self):
        """Vérifier la représentation string du mapping."""
        mapping = EdxUserMapping.objects.create(
            user_sis=self.user, username_edx="edx-user-str-test"
        )
    ...
    def test_unique_username_edx(self):
        ...
        EdxUserMapping.objects.create(
            user_sis=self.user, username_edx="unique-edx-user"
        )
        user2 = Utilisateur.objects.create_user(
            username="test_user_2", email="test2@test.com", password="testpass123"
        )
        with pytest.raises(IntegrityError):
            EdxUserMapping.objects.create(
                user_sis=user2, username_edx="unique-edx-user"  # Même username
            )
    ...
    def test_one_to_one_user_sis(self):
        ...
        with pytest.raises(IntegrityError):
            EdxUserMapping.objects.create(
                user_sis=self.user, username_edx="second-mapping"  # Même utilisateur
            )
    ...
    def test_mapping_with_edx_id(self):
        ...
        assert mapping.date_sync is not None

    def test_deactivate_mapping(self):
        """Désactiver un mapping."""
        mapping = EdxUserMapping.objects.create(
            user_sis=self.user, username_edx="to-deactivate", actif=True
        )
        mapping.actif = False
        mapping.save()
...
class OutboxEventTestCase(TestCase):
    ...
    def test_event_failure_and_retry(self):
        ...
        assert "timeout" in event.erreur.lower()
...
class IntegrationModelRelationsTestCase(TestCase):
    ...
    def test_cascade_delete_user_mapping(self):
        ...
        user = Utilisateur.objects.create_user(
            username="cascade_user", email="cascade@test.com", password="testpass123"
        )
        EdxUserMapping.objects.create(user_sis=user, username_edx="cascade-edx-user")
