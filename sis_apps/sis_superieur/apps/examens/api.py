...
from .models import SessionExamen, EpreuveExamen, ConvocationExamen
...
class SessionsExamenViewSet(viewsets.ModelViewSet):
    ...
    def epreuves(self, request, pk=None):
        ...
        epreuves = session.epreuves.select_related('ecue').order_by('date', 'heure_debut')
    ...
    def statistiques(self, request, pk=None):
        ...
        return Response({
            ...
            **totals,
            'taux_presence': round(totals['presents'] / totals['convoques'] * 100, 2) if totals['convoques'] else 0,
        })
...
class EpreuvesExamenViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return EpreuveExamen.objects.select_related('session', 'ecue').prefetch_related('surveillants')
    ...
    def convocations(self, request, pk=None):
        ...
        convocations = epreuve.convocations.select_related('etudiant__user').order_by('numero_place')
    ...
    def generer_convocations(self, request, pk=None):
        ...
        inscrits = InscriptionPedagogique.objects.filter(
            ecue=epreuve.ecue
...
class ConvocationsExamenViewSet(viewsets.ModelViewSet):
    ...
    def pointer(self, request, pk=None):
        ...
        return Response({'detail': f"Convocation mise à jour: {new_statut}", 'id': convocation.id})
