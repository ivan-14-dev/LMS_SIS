...
from .models import ProgrammeMobilite, CandidatureMobilite, AccordEtudes
...
class ProgrammeMobiliteListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'nom', 'type', 'type_display',
            'universite_accueil', 'pays', 'duree_mois',
            'nb_places', 'nb_candidatures', 'places_restantes',
            'date_limite_candidature', 'actif',
        ]
...
class ProgrammeMobiliteDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'nom', 'type', 'type_display',
            'universite_accueil', 'pays', 'duree_mois',
            'nb_places', 'langue_requise', 'niveau_langue',
            'description', 'date_limite_candidature',
            'annee_universitaire', 'annee_libelle',
            'actif', 'created_at',
        ]
...
class CandidatureMobiliteListSerializer(serializers.ModelSerializer):
    ...
    universite = serializers.CharField(source='programme.universite_accueil', read_only=True)
    ...
    class Meta:
        ...
        fields = [
            ...
            'programme', 'programme_nom', 'universite',
            'moyenne_ponderee', 'statut', 'statut_display',
            'date_soumission',
        ]
...
class CandidatureMobiliteDetailSerializer(serializers.ModelSerializer):
    ...
    programme_nom = serializers.CharField(source='programme.nom', read_only=True)
    statut_display = serializers.CharField(source='get_statut_display', read_only=True)
    decision_par_nom = serializers.CharField(source='decision_par.get_full_name', read_only=True)
    ...
    class Meta:
        ...
        fields = [
            ...
            'lettre_motivation', 'cv', 'releve_notes', 'certificat_langue',
            'projet_personnel', 'moyenne_ponderee',
            'statut', 'statut_display', 'date_soumission',
            'decision_par', 'decision_par_nom', 'motif_refus', 'created_at',
        ]
        read_only_fields = ['id', 'created_at']
...
class AccordEtudesSerializer(serializers.ModelSerializer):
    ...
    etudiant_nom = serializers.CharField(source='candidature.etudiant.user.get_full_name', read_only=True)
    programme_nom = serializers.CharField(source='candidature.programme.nom', read_only=True)
    statut_display = serializers.CharField(source='get_statut_display', read_only=True)
    
    class Meta:
        model = AccordEtudes
        fields = [
            'id', 'candidature', 'etudiant_nom', 'programme_nom',
            'pdf_signe', 'statut', 'statut_display',
            'date_validation_origine', 'date_validation_accueil',
        ]
        read_only_fields = ['id']
