...
class IsBibliothecaireOrReadOnly(IsAuthenticated):
    ...
    def has_permission(self, request, view):
        ...
        return user.is_staff or getattr(user, 'role', '') in (
            'bibliothecaire', 'responsable_bibliotheque'
        )
...
class EmpruntsViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = Emprunt.objects.select_related(
            'exemplaire__livre', 'emprunteur'
        )
        user = self.request.user
        # Un utilisateur ne voit que ses propres emprunts (sauf bibliothécaire)
        if not user.is_staff and getattr(user, 'role', '') not in ('bibliothecaire', 'responsable_bibliotheque'):
    ...
    def rendre(self, request, pk=None):
        ...
        if emprunt.date_retour_reelle > emprunt.date_retour_prevue:
            jours_retard = (emprunt.date_retour_reelle - emprunt.date_retour_prevue).days
    ...
    def renouveler(self, request, pk=None):
        ...
        if emprunt.nb_renouvellements >= 2:
            return Response({'error': "Limite de renouvellements atteinte."}, status=400)
        
        emprunt.date_retour_prevue += timedelta(days=14)
        emprunt.nb_renouvellements += 1
        emprunt.statut = 'renouvele'
        emprunt.save(update_fields=['date_retour_prevue', 'nb_renouvellements', 'statut'])
        return Response({
