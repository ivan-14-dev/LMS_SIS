...
class BilanECTSSerializer(serializers.ModelSerializer):
    ...
    formation_nom = serializers.CharField(source='inscription_admin.formation.nom', read_only=True)
    taux_validation = serializers.DecimalField(max_digits=5, decimal_places=2, read_only=True)
    ...
    class Meta:
        ...
        fields = [
            ...
            'inscription_admin', 'formation_nom',
            'credits_inscrits', 'credits_valides',
            'credits_compenses', 'credits_echec',
            'moyenne_ponderee', 'taux_validation',
            'ues_validees', 'ues_validees_list',
            'ues_compensees', 'ues_compensees_list',
            'ues_echec', 'ues_echec_list',
            'date_calcul', 'created_at',
        ]
        read_only_fields = ['id', 'date_calcul', 'created_at']
    
    def get_ues_validees_list(self, obj):
        return [{'id': ue.id, 'code': ue.code, 'nom': ue.nom, 'credits': ue.credits} for ue in obj.ues_validees.all()]
    
    def get_ues_compensees_list(self, obj):
        return [{'id': ue.id, 'code': ue.code, 'nom': ue.nom, 'credits': ue.credits} for ue in obj.ues_compensees.all()]
    
    def get_ues_echec_list(self, obj):
        return [{'id': ue.id, 'code': ue.code, 'nom': ue.nom, 'credits': ue.credits} for ue in obj.ues_echec.all()]
