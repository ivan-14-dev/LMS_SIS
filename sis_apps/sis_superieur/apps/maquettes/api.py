...
from apps.formations.models import MaquetteFormation, Formation
...
class MaquettesViewSet(viewsets.ModelViewSet):
    ...
    def par_formation(self, request):
        ...
        maquettes = self.get_queryset().filter(
    ...
    def par_semestre(self, request):
        ...
        if not formation_id or not semestre_id:
            return Response({'error': 'formation_id et semestre_id requis.'}, status=400)
