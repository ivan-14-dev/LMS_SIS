...
class AnneeScolaireFactory(factory.django.DjangoModelFactory):
    ...
    date_fin = factory.LazyAttribute(
        lambda o: o.date_debut + timezone.timedelta(days=300)
    )
    en_cours = True
...
class EleveFactory(factory.django.DjangoModelFactory):
    ...
    date_naissance = factory.LazyFunction(
        lambda: fake.date_of_birth(minimum_age=14, maximum_age=20)
    )
...
class EdxCourseMappingFactory(factory.django.DjangoModelFactory):
    ...
    course_name = factory.LazyAttribute(
        lambda o: f"{o.matiere.libelle} - {o.classe.libelle}"
    )
    actif = True
...
class EdxGradeLogFactory(factory.django.DjangoModelFactory):
    ...
    score = factory.LazyFunction(
        lambda: fake.pydecimal(min_value=0, max_value=20, right_digits=2)
    )
    max_score = 20
    completion = factory.LazyFunction(
        lambda: fake.pydecimal(min_value=0, max_value=100, right_digits=2)
    )
...
class OutboxEventFactory(factory.django.DjangoModelFactory):
    ...
    event_type = factory.Iterator(
        ["user.created", "user.updated", "enrollment.created", "grade.updated"]
    )
