...
from .serializers import (
    ...
    MembreClubSerializer,
    SeanceClubSerializer,
)
...
class ClubsViewSet(viewsets.ModelViewSet):
    ...
    def membres(self, request, pk=None):
        ...
        membres = club.membres.filter(statut='actif').select_related(
    ...
    def statistiques(self, request):
        ...
        by_type = Club.objects.filter(actif=True).values('type').annotate(
            count=Count('id')
        )
...
class MembresClubViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return MembreClub.objects.select_related(
            'club', 'eleve__user', 'eleve__classe'
        )
