...
def _scheduler_request(tenant):
    return SimpleNamespace(
        tenant=tenant,
        user=None,
        request_id="scheduler-secondaire-notes-submission-windows",
    )
...
def emit_submission_window_reminders():
    ...
    for tenant in tenant_model.objects.filter(actif=True):
        ...
        with schema_context(tenant.schema_name):
            ...
            evaluations = (
                Evaluation.objects.select_related("enseignant__user")
                .filter(fin_soumission__isnull=False, fin_soumission__gte=now, fin_soumission__lte=deadline)
            )
            request = _scheduler_request(tenant)
            for evaluation in evaluations:
                recipient = getattr(getattr(evaluation, "enseignant", None), "user", None)
