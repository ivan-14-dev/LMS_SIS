...
class WebhookHandler:
    ...
    def handle_course_published(self, payload: dict):
        """CMS a publié un cours : on marque la mapping comme actif."""
    ...
    def handle_xblock_published(self, payload: dict):
        """Un XBlock a été modifié dans Studio : notification."""
        course_key = payload.get("course_id")
        block_id = payload.get("block_id")
        logger.info(f"XBlock published: {block_id} in {course_key}")

    def handle_asset_uploaded(self, payload: dict):
        asset_id = payload.get("asset_id")
        course_key = payload.get("course_id")
        logger.info(f"Asset uploaded: {asset_id} in {course_key}")
