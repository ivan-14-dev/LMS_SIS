...
from .models import Eleve, Inscription, Tuteur, EleveTuteur
...
class EleveListSerializer(serializers.ModelSerializer):
    ...
    niveau_nom = serializers.CharField(source='classe_actuelle.niveau.nom', read_only=True)
    statut_display = serializers.CharField(source='get_statut_display', read_only=True)
    
    class Meta:
        model = Eleve
        fields = [
            'id', 'matricule', 'full_name', 'email', 'sexe',
            'classe_actuelle', 'classe_nom', 'niveau_nom',
            'statut', 'statut_display', 'bourse', 'transport', 'cantine',
        ]
...
class EleveDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Eleve
        fields = [
            'id', 'user', 'etablissement', 'matricule', 'ine',
            'full_name', 'first_name', 'last_name', 'email',
            'date_naissance', 'lieu_naissance', 'sexe', 'sexe_display',
            'nationalite', 'adresse', 'code_postal', 'ville',
            'classe_actuelle', 'classe_nom',
            'statut', 'statut_display', 'date_inscription', 'motif_sortie',
            'photo', 'qr_code',
            'bourse', 'transport', 'cantine', 'interne',
            'allergies', 'contact_urgence_nom', 'contact_urgence_tel',
...
class EleveCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'etablissement', 'date_naissance', 'lieu_naissance',
            'sexe', 'nationalite', 'adresse', 'code_postal', 'ville',
            'classe_actuelle', 'bourse', 'transport', 'cantine', 'interne',
        ]
...
class InscriptionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'classe', 'classe_nom', 'annee_scolaire', 'annee_libelle',
            'date_inscription', 'statut', 'statut_display', 'motif',
            'created_at',
        ]
...
class TuteurSerializer(serializers.ModelSerializer):
    ...
    lien_display = serializers.CharField(source='get_lien_parente_display', read_only=True)
    
    class Meta:
        model = Tuteur
        fields = [
            'id', 'etablissement', 'user', 'nom', 'prenom',
            'lien_parente', 'lien_display',
            'telephone', 'telephone_2', 'email', 'profession',
            'adresse', 'autorise_sortie', 'autorise_photos',
            'created_at',
        ]
        read_only_fields = ['id', 'created_at']
...
class EleveTuteurSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = EleveTuteur
        fields = [
            'id', 'eleve', 'eleve_nom', 'tuteur', 'tuteur_nom',
            'est_payeur', 'est_contact_urgence', 'autorise_acces_portail',
        ]
