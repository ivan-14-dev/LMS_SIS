...
from .models import Etablissement, AnneeScolaire, Periode


class EtablissementSerializer(serializers.ModelSerializer):
    """Serializer pour les établissements."""
    type_display = serializers.CharField(source='get_type_display', read_only=True)
    systeme_periodes_display = serializers.CharField(source='get_systeme_periodes_display', read_only=True)
    nb_annees = serializers.SerializerMethodField()
    
    class Meta:
        model = Etablissement
        fields = [
            'id', 'nom', 'type', 'type_display', 'uai',
            'adresse', 'code_postal', 'ville', 'pays',
            'telephone', 'email', 'site_web', 'logo', 'devise',
            'ministere_tutelle', 'systeme_periodes', 'systeme_periodes_display',
            'nb_annees', 'actif', 'date_creation',
        ]
...
class AnneeScolaireSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = AnneeScolaire
        fields = [
            'id', 'etablissement', 'libelle',
            'date_debut', 'date_fin',
            'en_cours', 'cloturee',
            'nb_periodes', 'created_at', 'updated_at',
        ]
