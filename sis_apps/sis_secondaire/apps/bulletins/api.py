...
from .serializers import (
    AppreciationMatiereSerializer,
    AppreciationMatiereCreateSerializer,
)
...
class IsEnseignantOrScolarite(IsAuthenticated):
    ...
    def has_permission(self, request, view):
        ...
        if request.method in ('GET', 'HEAD', 'OPTIONS'):
            return True
        user = request.user
        return user.is_staff or hasattr(user, 'enseignant_profile') or \
            getattr(user, 'role', '') in ('scolarite', 'directeur', 'proviseur', 'principal')
...
class AppreciationsMatiereViewSet(viewsets.ModelViewSet):
    ...
    def par_eleve(self, request):
        ...
        appreciations = self.get_queryset().filter(
            eleve_id=eleve_id, periode_id=periode_id
