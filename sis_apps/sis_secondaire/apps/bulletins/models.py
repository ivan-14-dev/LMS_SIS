...
class AppreciationMatiere(models.Model):
    ...
    eleve = models.ForeignKey(Eleve, on_delete=models.CASCADE, related_name="appreciations_matiere")
    matiere = models.ForeignKey(Matiere, on_delete=models.CASCADE, related_name="appreciations")
    periode = models.ForeignKey(Periode, on_delete=models.CASCADE, related_name="appreciations")
    appreciation = models.TextField()
    moyenne_matiere = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True)
    moyenne_classe = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True)
