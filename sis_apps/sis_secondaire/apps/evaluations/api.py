...
class IsEnseignantOrScolarite(IsAuthenticated):
    ...
    def has_permission(self, request, view):
        ...
        return (
            user.is_staff
            or hasattr(user, "enseignant_secondaire")
            or getattr(user, "role", "") in ("scolarite", "directeur", "proviseur")
        )
...
class EvaluationsViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Note.objects.select_related("eleve__user", "matiere", "periode")
    ...
    def par_classe(self, request):
        ...
        evaluations = (
            self.get_queryset()
            .filter(eleve__classe_id=classe_id)
            .order_by("-date")[:100]
        )
