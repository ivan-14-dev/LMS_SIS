...
from .models import Appel, Presence, Justificatif
...
class AppelListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Appel
        fields = [
            'id', 'creneau', 'creneau_libelle', 'date',
            'enseignant', 'enseignant_nom',
            'statut', 'statut_display',
            'nb_presents', 'nb_absents', 'date_saisie',
        ]
...
class PresenceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Presence
        fields = [
            'id', 'appel', 'eleve', 'eleve_matricule', 'eleve_nom',
            'statut', 'statut_display', 'retard_minutes', 'sortie_heure',
            'commentaire', 'notifie_parent', 'date_notification',
            'a_justificatif',
        ]
        read_only_fields = ['id']
...
class JustificatifSerializer(serializers.ModelSerializer):
    ...
    eleve_nom = serializers.CharField(source='presence.eleve.user.get_full_name', read_only=True)
    date_absence = serializers.DateField(source='presence.appel.date', read_only=True)
    statut_display = serializers.CharField(source='get_statut_display', read_only=True)
    valide_par_nom = serializers.CharField(source='valide_par.user.get_full_name', read_only=True)
    
    class Meta:
        model = Justificatif
        fields = [
            'id', 'presence', 'eleve_nom', 'date_absence',
            'motif', 'document', 'date_depot',
            'statut', 'statut_display',
