...
class PortailEleveViewSet(viewsets.ViewSet):
    ...
    def tableau_bord(self, request):
        ...
        notes_recentes = Note.objects.filter(
            eleve=eleve
        ).select_related('matiere').order_by('-date')[:5]
        ...
        return Response({
            ...
            } if eleve.classe else None,
            'statistiques': {
                'moyenne': float(eleve.moyenne_generale) if eleve.moyenne_generale else None,
                'rang': eleve.rang,
                'absences': absences,
            },
    ...
    def bulletins(self, request):
        ...
        bulletins = Bulletin.objects.filter(
            eleve=eleve
        ).select_related('periode').order_by('-periode__date_fin')
        
        return Response([{
            'id': b.id,
            'periode': str(b.periode),
            'moyenne': float(b.moyenne_generale) if b.moyenne_generale else None,
            'rang': b.rang,
    ...
    def emploi_du_temps(self, request):
        ...
        return Response([{
            ...
            'enseignant': c.enseignant.user.get_full_name() if c.enseignant else None,
    ...
    def absences(self, request):
        ...
        ).select_related('appel').order_by('-appel__date')[:30]
