...
from .serializers import SalleListSerializer, SalleDetailSerializer
...
class SallesViewSet(viewsets.ModelViewSet):
    ...
    def par_batiment(self, request):
        ...
        stats = self.get_queryset().values('batiment').annotate(
            count=Count('id')
        )
    ...
    def emploi_du_temps(self, request, pk=None):
        ...
        creneaux = Creneau.objects.filter(
            salle=salle, actif=True
        ).select_related('classe', 'matiere', 'enseignant__user').order_by('jour', 'heure_debut')
