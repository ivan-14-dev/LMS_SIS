...
from .serializers import (
    CreneauListSerializer,
    CreneauDetailSerializer,
    ContrainteSerializer,
)
...
class CreneauxViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = ['classe', 'matiere', 'enseignant', 'salle', 'jour', 'type', 'actif']
    ...
    def get_queryset(self):
        ...
        if hasattr(user, 'personnel_profile') and not self.request.query_params.get('all'):
    ...
    def par_enseignant(self, request):
        ...
        if not enseignant_id:
            ...
            if hasattr(request.user, 'personnel_profile'):
                ...
                return Response({'error': "Paramètre enseignant_id requis."}, status=400)
    ...
    def conflits(self, request):
        ...
        for (jour, debut, fin), crens in slots.items():
            ...
            for salle_id, salle_crens in salles.items():
...
class ContraintesViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Contrainte.objects.select_related(
            'enseignant__user', 'salle', 'classe'
        )
